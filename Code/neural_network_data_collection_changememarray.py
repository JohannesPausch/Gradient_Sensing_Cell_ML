...
for memory in memarrays:
    for init_distance in distances:
        final_counts =[]
        final_steps = []
        final_time = []
        for seed in seeds: 
            mem = 0.5*memory #time gradient 
        ...
        with open("NNCOUNTS_MEMARRAY" + str(memory) +".txt", "a") as output:
            output.write(str(init_distance)+'\n')
            output.write(str(final_counts)+'\n')
        with open("NNSTEPS_MEMARRAY" + str(memory) +".txt", "a") as output:
            output.write(str(init_distance)+'\n')
            output.write(str(final_steps)+'\n')
        with open("NNTIMESMEMARRAY" + str(memory) +".txt", "a") as output:
            output.write(str(init_distance)+'\n')
