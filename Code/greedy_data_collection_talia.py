    ...
    for seed in seeds: 
        ...
        while(count <= max_particles):
            ...
            if ind == -1: 
                ...
                ind = ind[0][0]
                time_move = tm - time_since 
                print('time since last moved =', time_move)
                received,source = mlbi.update_BrownianParticle(brownian_pipe,direction_sphcoords[ind][0],direction_sphcoords[ind][1], step_radius=0.01*time_move)
                if str(source[0]) == 'F':
                    print('# Source found')
                    break
                else:
                    ...
                    sourcez = source[0]* z
                    time_since = tm
                    print('store time of movement = ', time_since) #store this for next movement
