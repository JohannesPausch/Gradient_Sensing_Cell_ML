...
# takes azimuth phi (0,pi) and polar angle theta (0,2pi), turns them randomly and returns their turned version. 
...
def random_3d_rotation(phi,theta,use_seed=None):   
    try:
        np.random.seed(seed=use_seed)
        y=np.matmul(special_ortho_group.rvs(3),np.array([np.cos(theta)*np.sin(phi),np.sin(theta)*np.sin(phi),np.cos(phi)]))#special_ortho_group.rvs(3)
